...
class SentinelOneIngestion(SIEMIngestionService):
    ...
    def __init__(self) -> None:
        ...
        # Guards the restore_all_integration_secrets() reload below so it
        # runs at most once per instance instead of on every _credentials()
        # call while creds are empty (that reload decrypts the ENTIRE
        # secrets store across every configured integration, plus a DB
        # query via get_integration_config() -- unbounded per-call cost if
        # SentinelOne is genuinely unconfigured).
        self._secrets_reload_attempted = False
    ...
    def _credentials(self):
        ...
        # Daemon may start before the backend writes secrets.enc. If both
        # values are still empty, attempt one lazy reload from the
        # encrypted store -- but only once per instance; if it didn't help
        # the first time, retrying it on every subsequent call just re-pays
        # the full-store-decrypt cost for nothing.
        if (not url or not token) and not self._secrets_reload_attempted:
            self._secrets_reload_attempted = True
            try:
    ...
    async def fetch_alerts(
        ...
        if start_time:
            # %f gives the real microseconds (not a hardcoded .000000) so a
            # cursor advanced to e.g. 12:00:06.123456 by the federation
            # adapter doesn't get truncated back to the whole second on
            # every request -- inclusive createdAt__gte would otherwise
            # re-match and re-fetch threats already ingested in that second.
            base_params["createdAt__gte"] = start_time.strftime("%Y-%m-%dT%H:%M:%S.%fZ")
        if end_time:
            base_params["createdAt__lte"] = end_time.strftime("%Y-%m-%dT%H:%M:%S.%fZ")
    ...
    def transform_alert_to_finding(
        ...
        storyline_id = threat.get("storylineId") or threat_info.get("storylineId") or ""
        ...
        # ── agentRealtimeInfo ─────────────────────────────────────────────────
        hostname = (
            agent_info.get("agentComputerName") or threat.get("agentComputerName") or ""
        )
        os_type = agent_info.get("osType") or threat.get("osType") or ""
        os_name = agent_info.get("agentOsName") or ""
        os_version = (
            agent_info.get("agentOsRevision")
            or agent_info.get("osRevision")
            or threat.get("osRevision")
            or ""
        )
        ...
        last_logged_user = (
            ...
            or agent_detection.get("agentLastLoggedInUpn")
            or ""
        )
        username = (
            process_user
            or threat.get("username")
        ...
        if detection_type:
            desc_parts.append(f"Detection: {detection_type}")
        if hostname:
            desc_parts.append(f"Host: {hostname}")
        if os_type or os_name:
            desc_parts.append(f"OS: {os_name or os_type} {os_version}".strip())
        if username:
        ...
        if k8s_info:
            desc_parts.append(
                f"K8s: {k8s_info.get('namespace','')}/{k8s_info.get('pod','')}"
            )
        if container_info:
            desc_parts.append(
                f"Container: {container_info.get('name','') or container_info.get('image','')}"
            )
        desc_parts.append(
